...
public interface CombinedTrainDataRepository extends JpaRepository<WordsList, Long> {
    @Query(value =
            "SELECT id, ARRAY(SELECT jsonb_array_elements_text(words)) AS words, NULL as text, creation_time, user_id, 'WORDS_LIST' as type FROM words_list " +
            "UNION ALL " +
            "SELECT id, NULL as words, text, creation_time, user_id, 'TRAIN_TEXT' as type FROM train_text " +
